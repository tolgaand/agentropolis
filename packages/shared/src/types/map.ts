...
/**
 * Border padding for building placement (2-tile safety margin from parcel edges)
 * Fences are rendered at tiles 0 and 19, so buildings must stay within tiles 2-17
 */
...
export interface MapObject {
  ...
  isSystemOwned?: boolean;  // True for seed/founder buildings (monument, founder parcels)
}
...
// ============================================================================
// Ring-Based Village Layout
// ============================================================================
//
// 20×20 parcel organized as a medieval village with central plaza:
//
...
//  7   .  .  .  .  .  .  .  .  .  P  P  .  .  .  .  .  .  .  .  .
//  8   .  .  .  .  .  .  .  .  .  P  P  .  .  .  .  .  .  .  .  .
//  9   .  .  .  .  P  P  P  P  P  W  W  P  P  P  P  P  .  .  .  .
// 10   .  .  .  .  P  P  P  P  P  W  W  P  P  P  P  P  .  .  .  .
// 11   .  .  .  .  .  .  .  .  .  P  P  .  .  .  .  .  .  .  .  .
// 12   .  .  .  .  .  .  .  .  .  P  P  .  .  .  .  .  .  .  .  .
// 13   .  .  .  .  .  .  B  .  .  P  P  .  .  B  .  .  .  .  .  .
// 14   .  .  .  .  .  .  .  .  .  P  P  .  .  .  .  .  .  .  .  .
...
// 19   .  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .  .
//
// W = well (plaza center), P = path, B = building (clustered around center)
//
// Ring 0 (Center, tiles 8-11): Plaza area with well
// Ring 1 (Core, tiles 5-14): Main buildings clustered around plaza (2-4 tiles from center)
// Ring 2 (Mid, tiles 2-17): Secondary buildings at medium distance
// Ring 3 (Edge, tiles 0-1, 18-19): Border zone for fences/decorations

/**
 * Building placement slots - clustered around central plaza
 * priority 1 = closest to center (main buildings)
 * priority 2 = medium distance (secondary buildings)
 * priority 3 = edge positions (tertiary buildings)
 */
...
const PLACEMENT_SLOTS: PlacementSlot[] = [
  // Ring 1: Core positions (adjacent to plaza) - for main/large buildings
  // Safe zone: 2-17, large buildings (3x3) need positions 2-14
  { x: 6, y: 6, priority: 1, preferredSize: 'large' },   // NW of center (extends to 9,9)
  { x: 11, y: 6, priority: 1, preferredSize: 'large' },  // NE of center (extends to 14,9)
  { x: 6, y: 11, priority: 1, preferredSize: 'large' },  // SW of center (extends to 9,14)
  { x: 11, y: 11, priority: 1, preferredSize: 'large' }, // SE of center (extends to 14,14)

  // Ring 1.5: Close positions - for medium buildings (3x2)
  // Medium buildings need positions 2-14 for x, 2-15 for y
  { x: 9, y: 4, priority: 2, preferredSize: 'medium' },   // North (extends to 12,6)
  { x: 9, y: 13, priority: 2, preferredSize: 'medium' },  // South (extends to 12,15)
  { x: 4, y: 9, priority: 2, preferredSize: 'medium' },   // West (extends to 7,11)
  { x: 13, y: 9, priority: 2, preferredSize: 'medium' },  // East (extends to 16,11)

  // Ring 2: Medium distance - for small buildings (2x2)
  // Small buildings need positions 2-15
  { x: 4, y: 4, priority: 3, preferredSize: 'small' },   // NW (extends to 6,6)
  { x: 14, y: 4, priority: 3, preferredSize: 'small' },  // NE (extends to 16,6)
  { x: 4, y: 14, priority: 3, preferredSize: 'small' },  // SW (extends to 6,16)
  { x: 14, y: 14, priority: 3, preferredSize: 'small' }, // SE (extends to 16,16)
];

/**
 * Generate a village-like layout for a 20×20 parcel.
 *
 * Architecture:
 * 1. Central plaza with well at (9,9) - the town square
 * 2. Buildings clustered around the plaza in rings (not spread to corners)
 * 3. Simple + shaped paths from plaza to parcel edges (N, S, E, W)
 * 4. Buildings placed closer together for a settlement feel
 * 5. Border area (0-1, 18-19) reserved for fences
 *
 * Ring 0 (Center): Plaza with well
 * Ring 1 (Core): Main buildings adjacent to plaza
 * Ring 2 (Mid): Secondary buildings at medium distance
 * Ring 3 (Edge): Border zone for fences only
 */
export function generateParcelLayout(theme: ParcelTheme, seed: string): ParcelLayout {
  ...
  function canPlace(x: number, y: number, w: number, h: number): boolean {
    // Enforce border padding - buildings must not be within BORDER_PADDING tiles of any edge
    if (x < BORDER_PADDING || y < BORDER_PADDING) return false;
    if (x + w > PARCEL_SIZE - BORDER_PADDING || y + h > PARCEL_SIZE - BORDER_PADDING) return false;

    // Check occupancy grid
    for (let dy = 0; dy < h; dy++) {
      for (let dx = 0; dx < w; dx++) {
        const gx = x + dx, gy = y + dy;
        if (gx < 0 || gx >= 20 || gy < 0 || gy >= 20 || grid[gy][gx]) return false;
      }
    }
    return true;
  }

  // --- STEP 1: Reserve plaza center (9,9 to 10,10 for well + surrounding space) ---
  markOccupied(9, 9, 2, 2);

  // --- STEP 2: Reserve paths (1-tile wide + shaped paths from center to edges) ---
  // North path: x=9, y=0 to y=8
  for (let y = 0; y <= 8; y++) {
    markOccupied(9, y, 1, 1);
  }
  // South path: x=9, y=11 to y=19
  for (let y = 11; y < 20; y++) {
    markOccupied(9, y, 1, 1);
  }
  // West path: x=0 to x=8, y=9
  for (let x = 0; x <= 8; x++) {
    markOccupied(x, 9, 1, 1);
  }
  // East path: x=11 to x=19, y=9
  for (let x = 11; x < 20; x++) {
    markOccupied(x, 9, 1, 1);
  }

  // --- STEP 3: Select building sprite + type by size ---
  function getBuildingForSize(size: SizeKey): { spriteId: number; buildingType: string } {
    ...
    switch (size) {
      case 'small':
        spriteId = seededRandomChoice(rng, sprites.smallBuildings);
        break;
      case 'medium':
        spriteId = sprites.mediumBuildings.length > 0
          ? seededRandomChoice(rng, sprites.mediumBuildings)
          : seededRandomChoice(rng, sprites.smallBuildings);
        break;
      case 'large':
        spriteId = sprites.largeBuildings.length > 0
          ? seededRandomChoice(rng, sprites.largeBuildings)
          : seededRandomChoice(rng, sprites.smallBuildings);
        break;
      case 'xlarge':
        spriteId = sprites.xlargeBuildings.length > 0
          ? seededRandomChoice(rng, sprites.xlargeBuildings)
          : (sprites.largeBuildings.length > 0
              ? seededRandomChoice(rng, sprites.largeBuildings)
              : seededRandomChoice(rng, sprites.smallBuildings));
        break;
    }
  ...
  // --- STEP 4: Place buildings using ring-based placement ---
  const allBuildings: ParcelBuilding[] = [];
  const MAX_BUILDINGS = 7;

  // Sort slots by priority (1 = closest, 3 = farthest)
  const sortedSlots = [...PLACEMENT_SLOTS].sort((a, b) => a.priority - b.priority);

  for (const slot of sortedSlots) {
    if (allBuildings.length >= MAX_BUILDINGS) break;

    // Try preferred size first, then fallback to smaller sizes
    const sizesToTry: SizeKey[] = [];

    // Building size selection based on position and theme
    if (allBuildings.length === 0) {
      // First building: main building (largest available)
      sizesToTry.push('xlarge', 'large', 'medium', 'small');
    } else if (allBuildings.length <= 2) {
      // Second and third buildings: secondary (medium/large)
      sizesToTry.push('large', 'medium', 'small');
    } else {
      // Remaining buildings: smaller structures
      sizesToTry.push('medium', 'small');
    }

    let placed = false;
    for (const size of sizesToTry) {
      const [fw, fh] = SIZE_FOOTPRINT[size];

      if (canPlace(slot.x, slot.y, fw, fh)) {
        markOccupied(slot.x, slot.y, fw, fh);
        const { spriteId, buildingType } = getBuildingForSize(size);
        allBuildings.push({
          localX: slot.x,
          localY: slot.y,
          spriteId,
    ...
    // If we couldn't place at this slot, try next slot
    if (!placed) continue;
  }

  // Ensure we have at least one building (fallback)
  ...
  // --- STEP 5: Add well decoration at center plaza ---
  const decorations: ParcelDecoration[] = [
    {
      localX: 9,
      localY: 9,
      spriteId: SPRITE_IDS.WELL,
    },
  ];

  // --- STEP 6: Ground tiles (+ shaped paths) ---
